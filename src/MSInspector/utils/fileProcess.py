...
def headerModify_old(inf_tmp, outf_tmp, experiment_type_tmp, skyzip_file_dir_baseName, fileNameInferredExpStatusDic, fileNameEmptyStatusDic):
...
def headerModify(inf_tmp, outf_tmp, experiment_type_tmp, skyzip_file_dir_baseName, fileNameInferredExpStatusDic, fileNameEmptyStatusDic):
	...
	# Inferred experiment type from outf_tmp.
	# Read outf_tmp into a dataframe and keep all the values in the format of string.
	if os.path.splitext(outf_tmp)[1].lower() == '.tsv':
		sepType = '\t'
	else:
		sepType = ','
	df_tmp = pd.read_csv(outf_tmp, sep=sepType, header=0, converters={i: str for i in range(0, 100)})
	if df_tmp.shape[0] == 0:
		fileNameEmptyStatusDic.update({skyzip_file_dir_baseName:True})
		fileNameInferredExpStatusDic.update({skyzip_file_dir_baseName:None})
		skylineTemp_type = None
		...
		if exp4Stutus:
			...
			inferredExpType = 'exp2'
			skylineTemp_type = 'new'
		elif (not exp2NewTemplateSatuts) and (not exp4Stutus) and dayStatus:
			inferredExpType = 'exp5'
			skylineTemp_type = None
		elif (not exp2NewTemplateSatuts) and (not exp4Stutus) and analyteConcentrationStatus:
			if internalStandardConcentrationStatus:
				inferredExpType = 'exp1'
				skylineTemp_type = 'new'
			else:
				inferredExpType = 'exp3'
