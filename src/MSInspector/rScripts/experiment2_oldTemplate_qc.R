...
#plot_output <- "True"
#plot_output_dir <- "D:\\Skyline_analysis\\qcAssayPortal\\qcAssayPortal\\src\\qcAssayPortal\\rScripts\\test\\exp2_concentration_check\\tmp"
            ...
            for (indexLabel in 1:length(protein_list)) {
                ...
                if (FALSE) {
                    ...
                    if (nrow(fragment_ion_results_tmp4) > 0) {
                        ...
                        errorReason <- paste('When checking the injections at each concentration level, ', paste(errorReasonTmp, collapse = ', '), ', not all of the area ratios are within 30% of the mean.')
                        errorInfor <- paste(c(c(SkyDocumentName, errorType, errorSubtype, errorReason, input_protein_name, input_peptide_sequence, '', input_precursor_charge), rep('', colNumber-5)), collapse='\t')
                        cat(errorInfor)
                        cat('\n')
                    }
                        ...
                        if (nrow(fragment_ion_results_tmp_idividual) > 1) {
                            try ({
                                ...
                                if (nrow(thisPeptide_5) > 0) {
                                    ...
                                    errorReason <- paste('When checking samples with the concentrations of Low, Medium and High, ', paste(errorReasonTmp, collapse = ', '), '.', sep='')
                                    errorInfor <- paste(c(c(SkyDocumentName, errorType, errorSubtype, errorReason, input_protein_name, input_peptide_sequence, '', input_precursor_charge), rep('', colNumber-5)), collapse='\t')
                                    cat(errorInfor)
                                    cat('\n')
                                }
